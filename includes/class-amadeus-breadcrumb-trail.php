 ...
 */
function amadeus_get_breadcrumb_trail( $args = array() ) {
	$breadcrumb = apply_filters( 'amadeus_breadcrumb_trail_object', null, $args );

	if ( ! is_object( $breadcrumb ) ) {
		$breadcrumb = new Amadeus_Breadcrumb_Trail( $args );
	}
 ...
 * @access public
 */
class Amadeus_Breadcrumb_Trail {
	...
	public function __construct( $args = array() ) {
		...
		$this->args = apply_filters( 'amadeus_breadcrumb_trail_args', wp_parse_args( $args, $defaults ) );
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			foreach ( $this->items as $item ) {
				...
				if ( 1 === $item_position && 1 < $item_count ) {
					...
					$separator = sprintf( '<span class="amadeus-separator">%s</span>', '/' );
			...
			$breadcrumb = sprintf(
				'<%1$s role="navigation" aria-label="%2$s" class="amadeus-breadcrumb-trail amadeus-breadcrumbs" itemprop="breadcrumb">%3$s%4$s%5$s</%1$s>',
		...
		$breadcrumb = apply_filters( 'amadeus_breadcrumb_trail', $breadcrumb, $this->args );
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:',                               'amadeus-elementor' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'amadeus-elementor' ),
			'home'                => esc_html__( 'Home',                                  'amadeus-elementor' ),
			'error_404'           => esc_html__( '404 Not Found',                         'amadeus-elementor' ),
			'archives'            => esc_html__( 'Archives',                              'amadeus-elementor' ),
			// Translators: %s is the search query.
			'search'              => esc_html__( 'Search results for: %s',                'amadeus-elementor' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s',                               'amadeus-elementor' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Comment Page %s',                       'amadeus-elementor' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s',                             'amadeus-elementor' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s',                               'amadeus-elementor' ),
		...
		$this->labels = apply_filters( 'amadeus_breadcrumb_trail_labels', wp_parse_args( $this->args['labels'], $defaults ) );
	}
	...
	protected function set_post_taxonomy() {
		...
		$this->post_taxonomy = apply_filters( 'amadeus_breadcrumb_trail_post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
	}
	...
	protected function add_items() {
		...
		$this->items = array_unique( apply_filters( 'amadeus_breadcrumb_trail_items', $this->items, $this->args ) );
	}
				...
				if ( isset( $matches ) ) {
					...
					foreach ( $matches as $match ) {
						...
						if ( ! empty( $post_types ) ) {
							...
							// Core filter hook.
							$label = apply_filters( 'amadeus_post_type_archive_title', $label, $post_type_object->name );
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'amadeus-elementor' ) ) );
		}
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'amadeus-elementor' ) ) );
		}
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'amadeus-elementor' ) ) );
		}
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'amadeus-elementor' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'amadeus-elementor' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'amadeus-elementor' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'amadeus-elementor' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'amadeus-elementor' ) ) );
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag ) {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'amadeus-elementor' ) ) ) );
				}

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag ) {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'amadeus-elementor' ) ) ) );
				}

				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag ) {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'amadeus-elementor' ) ) ) );
				}
